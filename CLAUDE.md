...
- **Fontes no render: resolvido.** `fonts.googleapis.com`/`fonts.gstatic.com` estão fora da allowlist, mas os pacotes `@fontsource/fraunces` e `@fontsource/manrope` vêm pelo npm (que passa pelo proxy) e o `remotion/src/fontes.ts` importa os pesos do sistema (Fraunces 300 itálico, 400, 500, 600; Manrope 400 a 700). O render sai com as fontes reais; conferido nos três posts de exemplo.
- **Posts estáticos**: `remotion/src/PostInstagram.tsx` (4:5, rótulo + título com frase itálica dourada + cartão de chat do WhatsApp + assinatura) com os dados em `remotion/src/posts.ts`; exemplos e legendas em `projects/01-posts-instagram-exemplos/POSTS.md`. O brand book do Design System está em **português** desde 18/set/2026 (versão 5).
